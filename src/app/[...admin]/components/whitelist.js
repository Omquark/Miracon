...
export default function Whitelist() {
    ...
    const retrieveUser = async () => {
        ...
        if (!username || username.length < 4 || username.length > 16 || !/^[\w_]+$/.test(username)) {
            ...
            if (rawData.success === false || !rawData?.data?.player?.id) {
                if (userID) userID.value = '';
                toastMessage = `Failed to retrieve UUID of user ${username}`;
            } else {
                if (userID) userID.value = rawData.data.player.id;
    ...
    const showWhitelistModal = () => {
        ...
        setModalHeader('Create Whitelist Entry');
        setModalShown(true);
    };
